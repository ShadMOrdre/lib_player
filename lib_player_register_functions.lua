...
local function lib_player_globalstep(dtime)
  lib_player.player_loop(dtime)
  sprint_globalstep(dtime)
		...
		--  lib_player.seed = lib_player.seed + dtime
		--  math.randomseed(lib_player.seed)


end
minetest.register_globalstep(lib_player_globalstep)

local function lib_player_die_player(player)
	local name = player:get_player_name()
	if player_api.player_attached[name] ~= nil and player_api.player_attached[name] == true then
		local a = player_api.player_attached[name]
		a.object:set_detach()
...
minetest.register_on_dieplayer(lib_player_die_player)

local function lib_player_dignode(pos, node, digger)
  --print("on_dignode")
  ...
  if digger ~= nil and digger ~= "" then
    ...
    if minetest.registered_nodes[node.name] ~= nil then
      if minetest.registered_nodes[node.name]["skill"] ~= nil then
         lib_player.drop_item(pos,"experience:1_exp")
	...
	if dug % 100 == 0 then
		...
		for _,item in pairs(e) do
			lib_player.drop_item(ppos,item)
		end
	end
  end
...
minetest.register_on_dignode(lib_player_dignode)

local function lib_player_placenode(pos, node, placer)
...
minetest.register_on_placenode(lib_player_placenode)
...
--			y = -100
--		},
--		text = "lib_player_spawning_hud.png"
--	})
	local name = player:get_player_name()
	pd.load_player(name)
	-- set some defaults
...
--	lib_player.teleport(player,game_origin)
	minetest.after(3,lib_player.check_spawn,player)
end
