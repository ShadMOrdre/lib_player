function energy.update_energy(p,name,dtime)
	-- loop through all online players and check their movement and update their energy
		...
		--local timelast = pd.get(name,"timeofday")
		--local timeofday = minetest.get_timeofday()
		local timesince = minetest.get_timeofday() - pd.get(name,"timeofday")
		--local daylast = pd.get(name,"currentday")
		--local currentday = minetest.get_day_count()
		local daysince = minetest.get_day_count() - pd.get(name,"currentday")
		...
		if timesince >= 0.05 then
			if lastpos ~= nil then
					...
					local anim = player_api.get_animation(p)
					local adj = 0.2
					...
					if player_api.player_attached[name] == "boats:boat" and adj < 0 then
						adj = adj * 0.75
					end
	...
	--				end
					...
					if p_energy >= 20 then
						...
						if anim.animation == "lay" then
							...
							player_api.set_animation(p, "stand")
...
function energy.respawnplayer(player)
	...
	pd.set(name,"currentday",minetest.get_day_count())
