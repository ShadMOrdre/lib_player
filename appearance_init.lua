...
function appearance_update (name)
	...
	player_api.set_textures(player, {tmp_new_skin,tmp_armor,tmp_wielditem})
...
minetest.register_node("lib_player:wardrobe",{
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if minetest.get_node(pointed_thing.under).name == "lib_player:wardrobe" then
		...
		elseif minetest.get_node(pointed_thing.under).name == "lib_player:mirror" then
		...
		elseif minetest.get_node(pointed_thing.under).name ~= "lib_player:wardrobe"
		or minetest.get_node(pointed_thing.under).name ~= "lib_player:mirror" then
			if unode.name == "air" then
				minetest.set_node(pos, {name = "lib_player:wardrobe", param2 = facedir})
				minetest.set_node({x = pos.x, y = pos.y + 1, z = pos.z}, {name = "lib_player:mirror", param2 = facedir})
			else
	...
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
		local unode = minetest.get_node({x = pos.x, y = pos.y + 1, z = pos.z})
		if unode.name == "lib_player:mirror" then
...
minetest.register_node("lib_player:mirror",{
	...
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
		local unode = minetest.get_node({x = pos.x, y = pos.y - 1, z = pos.z})
		if unode.name == "lib_player:wardrobe" then
			minetest.set_node({x = pos.x, y = pos.y - 1, z = pos.z}, {name = "air"})
		end
	end,
})
