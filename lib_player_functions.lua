...
lib_player.pl_hooks = {}
function lib_player.player_loop(dtime)
	local p = minetest.get_connected_players()
	local reset_hooks = {}
	for k,hook in pairs(lib_player.pl_hooks) do
		lib_player.pl_hooks[k].timer = lib_player.pl_hooks[k].timer + dtime
		if lib_player.pl_hooks[k].timer >= lib_player.pl_hooks[k].timeout then
			for _, player in pairs(p) do
				local name = player:get_player_name()
				lib_player.pl_hooks[k].timer = 0
				lib_player.pl_hooks[k].func(player,name,dtime)
			end
		end 
	end
end

function lib_player.register_pl_hook(f,t)
	table.insert(lib_player.pl_hooks,{func=f,timeout=t,timer=0})
end

function lib_player.teleport(player,pos)
	local name = player:get_player_name();
	pd.set(name,"lastpos",pos)
	player:moveto(pos)
end

function lib_player.check_spawn(player)
	print("checking spawn")
	local count = 0
	local newpos = player:getpos()
	local badpos = false
	if lib_player.obj_stuck(player) == true then
		...
		local ab = lib_player.above_ground(newpos)
		...
		lib_player.teleport(player,newpos)
		--count = count + 1
		--badpos = true
...
function lib_player.obj_stuck(obj)
	local pos = obj:getpos()
	local pn = lib_player.get_obj_nodes(obj)
...
function lib_player.get_obj_nodes(obj)
	local pos = obj:getpos()
	local retval = {}
...
function lib_player.above_ground(pos)
	local step = 0
	local dest = 0
	if pos.y > 0 then
		step = -1
